...
import { Container, Point, SimpleRope, type Application, type Sprite } from "pixi.js";
import { emitterContainer } from "../emitterUtils";
import { ColorOverlayFilter } from "@pixi/filter-color-overlay";
import { AdvancedBloomFilter } from "@pixi/filter-advanced-bloom";

function firework_constructor(app: Application, res: Sprite,startx: number, starty: number){
    // 实现原理是使用rope绘制每一条烟花线
    ...
    const main_angles = [Math.PI/9,Math.PI/3,Math.PI*2/3,Math.PI,Math.PI*25/18,-Math.PI/6];
    let inter = 0;
    ...
    or_container.filters = [new AdvancedBloomFilter({
        blur: 1
    })];
        ...
        main_points.forEach((item,index)=>{
            main_points[index].map((p,i)=>{
                ...
                if(time>=random_fade_m[index]){
                    main_ropes[index].alpha = 1-1/random_fade_m[index]*(time-random_fade_m[index]);
                }
                if(time>=4.5){
                    mr_container.filters = [new AdvancedBloomFilter({bloomScale: 1-(1/4.5)*(time-4.5)})]
                }
                if(i<=count){
                    p.x += 2*Math.cos(main_angles[index]);
                    p.y += 2*Math.sin(main_angles[index]) + (time-i*0.1)*0.25 - 0.125;
                }
            })
        })
    }
        ...
        other_points.map((item,index)=>{
            other_points[index].map((p,i)=>{
                ...
                if(time>=random_fade_o[index]){
                    other_ropes[index].alpha = 1-1/random_fade_o[index]*(time-random_fade_o[index]);
                }
                if(time>=2.5){
                    or_container.filters = [new AdvancedBloomFilter({bloomScale: 1-(1/2.5)*(time-2.5)})]
                }
                if(i<=count){
                    ...
                    p.y += 2*Math.sin(other_angles[index]) + (time-i*0.1)*0.25 - 0.125;
                }
            })
        })
    }
