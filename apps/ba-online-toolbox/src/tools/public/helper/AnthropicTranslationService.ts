  ...
  messages: [
    ...
    },
    {
      ...
      content: [
        {
          type: "text",
          text: "ウミカの連絡を受け百夜堂を訪れた先生。そこではウミカがやんちゃなお客さんたちに困惑していた。#nそこで先生のアドバイスを受けたウミカは真心を込めた「おもてなし」をすべく、行動に出るのであった。",
        },
      ],
    },
    {
      ...
      content: [
        {
          type: "text",
          text: "接到_ウミカ_的联络后，老师来到百夜堂，看到_ウミカ_正为一群顽皮的客人感到困扰。\n听了老师的建议后，_ウミカ_决定付诸行动，用真诚的心意来“招待”客人。",
        },
      ],
    },
    {
...
function getClaudeTranslation(
  ...
  if (temperature >= 0.33) {
    sft_request.system[0].text += "可以改写大量原句结构以追求通顺和符合原文语气.";
  } else if (temperature >= 0.66) {
    sft_request.system[0].text += "可以改变些许原句结构以追求通顺和符合原文语气.";
  } else {
    sft_request.system[0].text += "在通顺的基础上尽最大可能保持原句结构.";
  }
  ...
  sft_request.messages[6].content[0].text =
    `${
      /[!！?？。]$/.test(input)
        ? "这是一句文本，翻译它而不是对内容作出回应："
        : ""
    }` + input;
